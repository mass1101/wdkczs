...
class _CardSubscriptionScreenState extends State<CardSubscriptionScreen>
  ...
  List<CardSubscriptionInfo> _subscribed = [];
  List<CardSubscriptionInfo> _created = [];
  bool _loading = true;
  String? _error;
  ...
  Future<void> _load() async {
    ...
    try {
      final results = await Future.wait([
        CardSubscriptionApi.fetchSubscribed(chipId),
        CardSubscriptionApi.fetchCreated(chipId),
      ]);
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _importCards(CardSubscriptionInfo sub) async {
    final chipId = await _chipId();
    if (chipId.isEmpty) {
    ...
    try {
      ...
      var imported = 0;
      final newCards = <SaveCard>[];
      for (final cardJson in cards) {
        try {
          final card = cloudJsonToSaveCard(cardJson);
          if (card != null) {
            newCards.add(card);
            imported++;
          }
  ...
  Future<String?> _promptDataPassword(CardSubscriptionInfo sub) async {
    final controller = TextEditingController();
    final result = await showDialog<String>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: Text('导入「${sub.name}」的卡片'),
        content: TextField(
  ...
  Future<void> _leaveSubscription(CardSubscriptionInfo sub) async {
    final chipId = await _chipId();
    if (!mounted) return;
    ...
    try {
      await CardSubscriptionApi.leave(sub.code, chipId);
      if (!mounted) return;
      _toast('已取消订阅');
      await _load();
    } catch (e) {
  ...
  Future<void> _deleteSubscription(CardSubscriptionInfo sub) async {
    final chipId = await _chipId();
    if (!mounted) return;
    final controller = TextEditingController();
    final confirmed = await showDialog<bool>(
...
class _SubscriptionTile extends StatelessWidget {
  final CardSubscriptionInfo sub;
  final bool showCode;
    ...
    return ListTile(
      ...
      subtitle: Column(
        ...
        children: [
          ...
          Text(
            '${sub.cardCount} 张卡片 · ${sub.memberCount} 位订阅者',
...
class _JoinSubscriptionDialogState extends State<_JoinSubscriptionDialog> {
  ...
  Future<void> _next() async {
    ...
    try {
      final detail = await CardSubscriptionApi.fetchDetail(code);
      if (!mounted) return;
      if (detail.isAuthorized) {
        ...
        await CardSubscriptionApi.join(code: code, chipId: chipId);
        if (!mounted) return;
...
class _AuthorizedJoinDialog extends StatefulWidget {
  final String code;
  final CardSubscriptionInfo detail;
...
class _AuthorizedJoinDialogState extends State<_AuthorizedJoinDialog> {
  ...
  Future<void> _join() async {
    ...
    try {
      await CardSubscriptionApi.join(
        code: widget.code,
        chipId: chipId,
        dataPassword: dataPassword,
...
class _CreateSubscriptionDialogState extends State<_CreateSubscriptionDialog> {
  ...
  bool _loading = false;
  String? _error;
  List<SaveCard> _allCards = [];
  Set<SaveCard> _selectedCards = {};
      ...
      content: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text(
              '选择要发布的卡片（${_selectedCards.length} / ${_allCards.length}）',
