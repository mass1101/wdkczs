...
- Instructions:
  ...
  - **围栏跟随的权限缺口**：定位权限被拒时 Dart 位置流静默不启动，表现为「跟随毫无反应」且无任何提示。已在 `_locateMe` 失败分支调 `_promptLocationPermission()`：SnackBar + 「去设置」→ `Geolocator.openAppSettings()`，回来后重查 `checkPermission` 并补 `_pos.start()`。权限判定用显式枚举比较（`always || whileInUse`）——`geolocator` 的 `LocationPermission` 没有 `isGranted` 扩展；`permission_handler` 虽在 pubspec 写 `^13.0.2` 实际解析到 12.0.3，其 `isGranted` 是 extension getter 不是方法。
