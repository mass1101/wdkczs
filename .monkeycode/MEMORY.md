...
- Instructions:
  ...
  - 排查对话框渲染类问题的可靠手段：写临时 widget test 直接 `pumpWidget(MaterialApp(theme: AppTheme.build(), ...))` 后 `showDialog(builder: CardEditDialog(card: sample))`，遍历全部 `TagType` 并打印 `tester.takeException()` 与 `find.byType(AlertDialog).evaluate().length`——length=0 即对话框没建出来，比看截图快。临时测试放 `test/` 下跑完 `mv` 到 `/tmp/opencode/`（勿 rm）。注意 `flutter test` 不会因 `takeException` 消费的异常而失败，须自己打印断言。
  ...
  - 诊断栏现有行（勿加回蓝点行）：设备连接状态 / 定位时间或「暂无定位」/ 坐标 / 命中围栏 / 卡片上传状态 / 滚动码状态。左上角另有「电子围栏」标题 + 状态文字 + 总开关。
