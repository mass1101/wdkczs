...
- Instructions:
  ...
  - 推送方式：`C=$(grep -m1 '^https://[^@]*@github.com$' /root/.git-credentials) && git -c credential.helper= push "${C}/mass1101/wdkczs.git" main`；gitee 把 host 换成 `gitee.com`、仓库路径换成 `zzx1101/NFCapp.git`。
  - 远端：github=`https://github.com/mass1101/wdkczs.git`、gitee=`https://gitee.com/zzx1101/NFCapp.git`，默认分支均 `main`；环境无 `ssh` 二进制，只能走 HTTPS。
...
- Category: Troubleshooting & Debugging | Environment Configuration
- Instructions:
  - **围栏坐标系不变量（定位不准的根因）**：地图瓦片是高德 GCJ-02，用户在图上点选的多边形存的是 GCJ-02；定位原始值是 WGS84，转换只能发生一次，且必须在匹配前。转换点在原生 `android/app/src/main/kotlin/com/z/nfc/GeofenceService.kt:245`（`wgs84ToGcj02` 后再 `findMatchingFence`），回调 Flutter 的 `onPosition`/`onFenceEvent` 坐标已是 GCJ-02。围栏**判定**只有原生一条链路，`GeofenceMatcher` 的 Dart 实现只用于地图上的图形交互。Dart 侧跑 `geolocator` 的硬前提：**写入 `_lastPosition` 前必须先转 GCJ-02**——原始 WGS84 直写会覆盖 `_lastPosition`，造成蓝点周期回跳几百米、并拿 WGS84 比 GCJ-02 多边形（境内偏移约 300–700 米）造成"人在围栏内不触发/不在却触发"。现已有一条合法的 Dart 流 `PositionProvider.start()`（`LocationAccuracy.high` + `distanceFilter: 2`，`lib/services/position_provider.dart`），内部先 `CoordinateConverter.wgs84ToGcj02` 再 `update(LatLng)`，只供地图蓝点/坐标行/地图跟随/诊断栏显示，不参与判定、不写 `GeofenceProvider.lastPosition`。旧的 `lib/services/location_service.dart` 未做坐标转换且与原生轮询并存，仍是禁止项，勿改回使用。
  ...
  - **定位排障（两个已踩过的根因）**：① 清单权限——`ACCESS_FINE_LOCATION` 曾带 `android:maxSdkVersion="30"`，API 31+ 只剩粗定位，原生 `requestLocationUpdates(GPS_PROVIDER)` 抛 `SecurityException` 被静默吞掉，Dart 侧 `getCurrentPosition` 也退化成网络粗定位（误差数百米）；已去掉该上限。② geolocator Android provider——`LocationSettings` 在 Android 默认走 Google FusedLocationProvider，国产无 GMS 设备上位置流会静默不回调（`getCurrentPosition` 可能只返回缓存的旧位置，表现为「有值但一直不更新」），`distanceFilter > 0` 在静止时同样不回调，而 `onError` 写成空闭包会把 `LocationServiceDisabledException`（系统定位总闸关闭）彻底吞掉，用户零提示。修法（`PositionProvider`）：`AndroidSettings(forceLocationManager: true, distanceFilter: 0, intervalDuration: 2s)` + 启动前 `isLocationServiceEnabled()` 检查 + 错误写 `lastError` 并 notify + `Timer.periodic(5s)` 调 `getCurrentPosition` 兜底；CU `location_service.dart` 用 `AndroidSettings` + `Timer` 轮询是同一思路。排查顺序：先查清单权限 → 再查 `isLocationServiceEnabled` → 再查 provider 与错误是否被吞。
  - `GeofenceService` 声明曾缺 `android:stopWithTask="false"`，应用退后台即被杀，表现同样是「暂无定位」。已对齐 CU 补齐，并补 `PROPERTY_SPECIAL_USE_FGS_SUBTYPE` 属性。
  - 围栏**定位显示与围栏判定已彻底分家（两条独立链路，勿再合并）**：「暂无定位」只表示原生围栏判定服务未跑（启动条件 = 总开关开 + NFC 设备已连接，`_enabled = _userEnabled && _isConnected`），与定位显示无关。定位显示只读 `AppScope.instance.controller.position`（`PositionProvider`，`start()` 进围栏页即开、引用计数、不检查总开关与设备连接）；`GeofenceProvider.lastPosition` 只由原生 channel 写，供悬浮窗与围栏状态用。两个 Provider 各自把 WGS84 转成 GCJ-02 后才落字段，互不写对方状态，判定只由原生服务驱动；一次性取位统一为 `PositionProvider.refresh()`（列表页与编辑页不再各自实现 `_getGcjPosition`，编辑页 `position` 参数已改为必填）。总开关关闭提示文案应说「围栏判定已停止」，设备未连时说「已开启：定位持续更新，连接设备后才判定」，别让"暂无定位"被误读成定位坏了。曾加过显示蓝点坐标与来源的诊断行，用户要求已移除，勿再加回。
  - **地图跟随按钮「打开后不跟随」的根因**：位置只在 `build()` 里用 `latest != _currentPosition` + `addPostFrameCallback` 作为副作用消费，而 `_currentPosition` 在同一次 build 内已被赋值，且位置源在设备未连时（原生服务不跑）根本不更新——两条叠加导致跟随永不触发。修法：跟随逻辑移出 `build()` 到定位回调（`PositionProvider` 的 `addListener` 里，位置变化即 `setState` + `_followMe && _mapTo(pos)`），并在 initState 启动 `PositionProvider` 使位置不依赖设备；`_mapTo` 用 `_mapReady` 守卫并保留当前 zoom（首次「定位我」仍 zoom 16）。围栏状态回调 `_onChange` 只做 `setState(() {})` 刷新界面，不再消费位置。**凡「数据变化后需要移动地图/滚动列表」的逻辑都应写在数据回调里，不要塞进 `build()`**。
  - **flutter_map 7.0.2 跟随机制已用 widget test 实证可用**：`onMapReady` 由 `widget.dart:64` 的 `addPostFrameCallback` 触发（首帧后即可置位守卫）；`MapController.move(LatLng, double)` 返回 `bool`，中心/缩放与当前相同时直接 no-op 不发射事件；`didUpdateWidget` 仅在 `options` 变更时调 `controller.options=` setter，而 `MapCamera.withOptions` 在 crs/minZoom/maxZoom 未变时 `return this`——所以把 `initialCenter: _currentPosition` 随 setState 反复重建 `MapOptions` **不会把地图拉回**，也无相机漂移。验证跟随是否真移动：`MapOptions.onMapEvent` 收 `MapEventMove` 后比对 `event.camera.center`（7.0.2 无 `onPositionMove`）。
  - **围栏跟随的权限/定位服务缺口**：定位权限被拒或系统定位总闸关闭时，位置流静默不启动，表现为「跟随毫无反应」且无任何提示。已在 `_locateMe` 失败分支按 `_pos.permissionGranted` 分流：未授权 → `_promptLocationPermission()`（SnackBar + 「去设置」→ `Geolocator.openAppSettings()`，回来后重查 `checkPermission` 并补 `_pos.start()`）；已授权但拿不到位置 → `_promptLocationService()`（`Geolocator.openLocationSettings()`）。权限判定用显式枚举比较（`always || whileInUse`）——`geolocator` 的 `LocationPermission` 没有 `isGranted` 扩展；`permission_handler` 虽在 pubspec 写 `^13.0.2` 实际解析到 12.0.3，其 `isGranted` 是 extension getter 不是方法。
  - **`GeolocatorPlatform` 可打桩**（供以后写围栏/定位单测）：`class X extends PlatformInterface with MockPlatformInterfaceMixin implements GeolocatorPlatform` + `X() : super(token: Object())`（必须调 super，否则隐式构造抛 NoSuchMethodError）+ 覆写 `checkPermission/requestPermission/getPositionStream`，其余成员加 `dynamic noSuchMethod` 兜底（否则编译报未实现）。测试里 `GeolocatorPlatform.instance = X()` 即可；`getPositionStream` 直接委托无 enabled 检查。`LocationPermission` 无 `isGranted`。
  - **对话框「显示为灰色空白」= build 抛异常，不是主题问题**：`CardEditDialog` 的 `_counterCtrls` 曾声明为 `late List<TextEditingController>`，而 `_initCounterControllers()` 先遍历该字段（dispose 旧控制器）再赋值——`initState` 首次调用时字段未初始化，抛 `LateInitializationError`，整个 `AlertDialog` 未构建（release 下表现即灰色空白框）。已改为 `final List<TextEditingController> _counterCtrls = []` + `.clear()`（对齐 CU `edit.dart:36` 的 `= []` 内联初始化）。**凡「late 字段 + 方法内先读后写」组合都是同类隐患**，改动时务必让声明处带初值。
  - 排查对话框渲染类问题的可靠手段：写临时 widget test 直接 `pumpWidget(MaterialApp(theme: AppTheme.build(), ...))` 后 `showDialog(builder: CardEditDialog(card: sample))`，遍历全部 `TagType` 并打印 `tester.takeException()` 与 `find.byType(AlertDialog).evaluate().length`——length=0 即对话框没建出来，比看截图快。临时测试放 `test/` 下跑完 `mv` 到 `/tmp/opencode/`（勿 rm）。注意 `flutter test` 不会因 `takeException` 消费的异常而失败，须自己打印断言。**循环遍历多卡型/多实例时必须给被测组件加 `key: ValueKey(...)` 强制重建 State**——`pumpWidget` 对同位置同类型组件是原地更新、复用旧 State，`initState` 不会重跑，`_card = widget.card` 这类只在 initState 取值的字段会沿用上一个实例的旧数据（表现为全部分支都看到第一份数据，断言必然失败且无任何异常）。`_DialogHost` 包 `showDialog` 同样受此影响，且旧的 dialog route 会逐轮堆积（`find.byType(ActionChip)` 数量倍增）。
  - 该 SDK 的 `DropdownButtonFormField` 用 `initialValue`（`value` 已废弃，SDK 提示 "Use initialValue instead"），`TextFormField` 同理；写对话框时用错参数名 analyze 会报错，勿按旧文档用 `value`。
  ...
  - 诊断栏现有行（勿加回蓝点行）：设备连接状态 / 定位时间或「暂无定位」/ 坐标 / 命中围栏 / 最新事件（命中·离开）/ 卡片上传状态 / 滚动码状态，末尾有「事件 N」按钮打开围栏事件列表对话框（可清空日志）。左上角另有「电子围栏」标题 + 状态文字 + 总开关。事件来源 = `GeofenceProvider._handleMatchedFence` 的 `_log('命中围栏 <名>')` / `_log('离开围栏 <名>')`，`eventLogs` getter 按关键字过滤 `_logs`（内存态，未落 SharedPreferences，重启即丢）。`_diagRow` 带可选 `maxWidth`（默认 200）+ `TextOverflow.ellipsis`，防超长围栏名把面板撑破。
...
- Instructions:
   ...
   - 卡库查看对话框导出入口的用户定版（覆盖 CU 的「bin/json 二选一对话框」）：按钮文案 = MIFARE Classic 四种卡型（1K/2K/4K/Mini，`isMifareClassic`）显示「导出 bin」，其余全部卡型显示「导出 json」；bin 走 `cardSaveToBin`（`card_backup.dart`，Classic 逐块拼字节）+ `FilePicker.saveFile` 存 `<名或UID>.bin`，json 走 `card.toJson()` utf8 编码存 `.json`，取消/无数据/异常都要提示。按钮名与文件格式是用户明确指定的，勿按 CU 改回。卡库更多菜单的「卡片分析」入口用户要求移除，`card_analyze_screen.dart` 已删除，勿再加回。
