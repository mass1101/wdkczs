...
- Category: Troubleshooting & Debugging | Environment Configuration
- Instructions:
  - **围栏坐标系不变量（定位不准的根因）**：地图瓦片是高德 GCJ-02，用户在图上点选的多边形存的是 GCJ-02；定位原始值是 WGS84，转换只能发生一次，且必须在匹配前。转换点在原生 `android/app/src/main/kotlin/com/z/nfc/GeofenceService.kt:245`（`wgs84ToGcj02` 后再 `findMatchingFence`），回调 Flutter 的 `onPosition`/`onFenceEvent` 坐标已是 GCJ-02。围栏**判定**只有原生一条链路，`GeofenceMatcher` 的 Dart 实现只用于地图上的图形交互。Dart 侧跑 `geolocator` 的硬前提：**写入 `_lastPosition` 前必须先转 GCJ-02**——原始 WGS84 直写会覆盖 `_lastPosition`，造成蓝点周期回跳几百米、并拿 WGS84 比 GCJ-02 多边形（境内偏移约 300–700 米）造成"人在围栏内不触发/不在却触发"。现已有一条合法的 Dart 流 `GeofenceProvider.startMapPositionStream()`（`LocationAccuracy.high` + `distanceFilter: 2`），内部先 `CoordinateConverter.wgs84ToGcj02` 再走统一入口 `_updatePosition`，只供地图蓝点/坐标行/地图跟随/悬浮窗显示，不参与判定。旧的 `lib/services/location_service.dart` 未做坐标转换且与原生轮询并存，仍是禁止项，勿改回使用。
  ...
  - 「暂无定位」= 原生围栏判定服务未跑（其启动条件 = 总开关开 + NFC 设备已连接，`_enabled = _userEnabled && _isConnected`）。这与「定位显示」是两件事：定位显示走 Dart 地图流，进围栏页即启动、不需要设备。总开关关闭提示文案应说「围栏判定已停止」，设备未连时说「已开启：定位持续更新，连接设备后才判定」，别让"暂无定位"被误读成定位坏了。
  - 围栏**定位显示与围栏判定已解耦**：诊断栏「定位」「坐标」行、地图蓝点、地图跟随、悬浮窗位置统一读 `_geo.lastPosition`，两个写入源（原生 channel / Dart 地图流）在入口 `_updatePosition(LatLng)` 处**都已是 GCJ-02**，故同一字段可直接混用，不再需要 `_getGcjPosition()` 兜底路径。判定只由原生服务驱动，Dart 流即使跑着也不影响命中/离开。曾加过显示蓝点坐标与来源的诊断行，用户要求已移除，勿再加回。
  - **地图跟随按钮「打开后不跟随」的根因**：位置只在 `build()` 里用 `latest != _currentPosition` + `addPostFrameCallback` 作为副作用消费，而 `_currentPosition` 在同一次 build 内已被赋值，且位置源在设备未连时（原生服务不跑）根本不更新——两条叠加导致跟随永不触发。修法：跟随逻辑移出 `build()` 到 `_onChange`（`ProviderSubscription` 回调内，位置变化即 `setState` + `_followMe && _mapTo(pos)`），并在 initState 启动 Dart 地图流使位置不依赖设备；`_mapTo` 用 `_mapReady` 守卫并保留当前 zoom（首次「定位我」仍 zoom 16）。**凡「数据变化后需要移动地图/滚动列表」的逻辑都应写在数据回调里，不要塞进 `build()`**。
