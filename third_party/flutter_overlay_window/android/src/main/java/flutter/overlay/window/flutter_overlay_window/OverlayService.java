...
public class OverlayService extends Service implements View.OnTouchListener {
    ...
    public static void updateStatus(String tag) {
        status += ";" + tag;
    }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        try {
            ...
            if (engine == null) {
                Log.e("OverlayService", "Cached engine is null, creating a new one");
            ...
            WindowManager.LayoutParams params = new WindowManager.LayoutParams(
                    WindowSetup.width == -1999 || WindowSetup.width == -1 ? WindowSetup.width : dpToPx(WindowSetup.width),
                    WindowSetup.height != -1999 && WindowSetup.height != -1 ? dpToPx(WindowSetup.height) : screenHeight(),
                    0,
                    -statusBarHeightPx(),
            ...
            updateStatus("window_added");
            reportInfo("悬浮窗窗口已添加");
            FlutterEngine statusEngine = engine;
            new Handler(android.os.Looper.getMainLooper()).postDelayed(() -> {
                boolean executing = statusEngine != null && statusEngine.getDartExecutor().isExecutingDart();
                updateStatus("dart_executing=" + executing);
            }, 2000);
        } catch (Exception e) {
            Log.e("OverlayService", "Failed to show overlay", e);
            updateStatus("error=" + (e.getMessage() != null ? e.getMessage() : e.toString()));
            reportError("showOverlay failed: " + e.getMessage());
            isRunning = false;
        }
        return START_STICKY;
    }

    private FlutterEngine createOverlayEngine() {
        FlutterEngineGroup engineGroup = new FlutterEngineGroup(this);
        DartExecutor.DartEntrypoint entryPoint = new DartExecutor.DartEntrypoint(
                FlutterInjector.instance().flutterLoader().findAppBundlePath(),
                "overlayMain"
        );
        return engineGroup.createAndRunEngine(this, entryPoint);
    }
    ...
    private void reportError(String message) {
        try {
            if (WindowSetup.messenger != null) {
                ...
                map.put("type", "overlay_error");
                map.put("message", message);
                WindowSetup.messenger.send(map);
            }
    ...
    private void bringMainToForeground() {
        try {
    ...
    public void onCreate() {
        try {
            ...
            if (flutterEngine == null) {
                ...
                flutterEngine = createOverlayEngine();
            ...
            if (flutterEngine != null) {
                ...
                overlayMessageChannel.setMessageHandler((message, reply) -> {
                    if (message != null) {
                        ...
                        if ("overlay_boot".equals(m)) {
                            updateStatus("ui_boot");
                        } else if ("overlay_ready".equals(m)) {
                            updateStatus("ui_ready");
                        } else if ("overlay_boot_error".equals(m)) {
                            updateStatus("ui_boot_error");
                        } else if ("pong".equals(m)) {
                            updateStatus("ui_alive");
                        } else if ("close".equals(m)) {
                            updateStatus("ui_close");
                            bringMainToForeground();
                            stopSelf();
                        } else if ("restore".equals(m)) {
                            updateStatus("ui_restore");
                            bringMainToForeground();
                        }
                    }
                ...
                try {
                    overlayMessageChannel.send("ping", new BasicMessageChannel.Reply<Object>() {
                        @Override
        ...
        startForeground(OverlayConstants.NOTIFICATION_ID, notification);
            ...
            updateStatus("create_error=" + (e.getMessage() != null ? e.getMessage() : e.toString()));
            reportError("overlay service onCreate failed: " + e.getMessage());
            stopSelf();
        }
    }
